...
// testing if
test("all the employees returned by the search have the specified uf", () => {
  const data = app.searchEmployeesByUf(employee.uf);
  const returnedEmployees = data['employees'];
  const msg = data['msg'];

  // verifies if all the employees returned by the search have the specified uf
  returnedEmployees.forEach((returnedEmployee) => {
    expect(returnedEmployee.uf).toEqual(employee.uf);
  });
...
// testing if
test("the search returns all the employees with the specified uf", () => {
  const data = app.searchEmployeesByUf(employee.uf);
  const returnedEmployees = data['employees'];
  const msg = data['msg'];

  // verifies if the search returns all the employees with the specified uf
...
test("no employee is returned by the search", () => {
  const data = app.searchEmployeesByUf("UF");
